...
type Props = PlanPickerBlockProps & Pick<Dialect, 'signature' | 'eyebrow'>
 ...
 */
export const PlanPickerClient: React.FC<Props> = ({
  heading,
  intro,
  plans,
  form,
  ...
  return (
    <div className="container">
      <div className="rounded-band bg-secondary/70 p-6 md:p-12">
              ...
              <fieldset>
                ...
                <div className="flex flex-wrap gap-2">
                  {needs.map((value) => (
                    <label
                      key={value}
                      className="cursor-pointer select-none rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-medium transition-colors hover:border-primary/50 has-[:checked]:border-primary has-[:checked]:bg-primary has-[:checked]:text-primary-foreground has-[:focus-visible]:ring-4 has-[:focus-visible]:ring-ring/20"
                    >
                      <input
                        type="radio"
                        name={`${uid}-need`}
                ...
                <SignatureCard signature={signature}>
                  ...
                  {belowMin && (
                    <p className="mt-4 rounded-lg bg-secondary px-3 py-2 text-xs leading-relaxed text-secondary-foreground">
                      {match.title} er typisk fra {minOf(match)} personer – er I færre, finder vi en
                      løsning sammen.
                    </p>
                  )}
                  <div className="mt-5">
